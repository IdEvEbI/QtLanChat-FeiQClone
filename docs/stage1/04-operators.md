...
- **前置自增**：`++x` - 先加 1 再使用，就像先升级再使用
- **后置自增**：`x++` - 先使用再加 1，就像使用完再升级
- **前置自减**：`--x` - 先减 1 再使用，就像先降级再使用
...
- **自增自减运算符**：用于递增或递减变量
  - **前置自增**：`++value` 先加 1 再返回，就像先升级再使用新功能
...
> **注意**：自增自减运算符属于单目运算符，优先级比乘除运算还高，就像必须先准备好（递增）再使用。
...
// 自增自减运算符优先级示例
int x = 5;
int y = ++x * 2;                // 先算++x（x变为6），再算 6*2 = 12
int z = x++ + 5;                // 先算x++（使用6，x变为7），再算 6+5 = 11
```

> **重要提示**：
>
> - 当不确定优先级时，使用括号明确运算顺序，就像数学中括号可以让计算更清晰
> - 自增自减作为单目运算符，优先级很高，仅次于括号
...
>
> **📌 新知识点预览 - if-else 语句**：代码中的 `if (条件) { ... } else { ... }` 是条件分支语句，用于根据条件选择执行不同的代码。
>
> - **`if (条件)`**：当条件为真时执行大括号内的代码，就像"如果...那么..."
> - **`else`**：当条件为假时执行 else 后面的代码，就像"否则..."
>
> **类比说明**：就像做选择题，"如果成绩大于 60 分，输出'及格'，否则输出'不及格'"。这就像生活中做决定："如果天气好，就去公园，否则就待在家"。
>
> 这是 C++ 中最重要的控制结构之一，我们将在 [控制结构详解](./05-control-structures.md) 中深入学习。现在只需要了解：程序可以根据不同的条件执行不同的代码。
