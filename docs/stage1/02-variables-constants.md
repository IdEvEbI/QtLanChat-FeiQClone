...
- **有什么优势**：类型安全（防止数据混乱）、内存管理（自动清理）、代码可读性（见名知义）

#### 2.1.3 关键特性

1. **先声明后使用**：C++ 要求变量必须先声明后使用，这是类型安全的基础。就像使用工具前要先从工具箱里拿出来
2. **类型安全**：每个变量都有明确的数据类型，防止不同类型的数据混在一起
3. **作用域管理**：变量只在特定范围内可见，就像房间里的东西只在房间里能用
4. **内存管理**：自动分配和释放内存，程序员不需要手动管理
5. **常量保护**：防止意外修改重要数据，就像给重要文件加锁
