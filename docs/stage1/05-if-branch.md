...
> 语法说明：如果条件为真，返回 `值1`；否则返回 `值2`。这是 if-else 的简化形式，只能用于赋值或返回值。

**类比**：就像"如果天气好就带伞，否则带雨衣"，用三元运算符就是 `天气好 ? 伞 : 雨衣`，伪代码示例如下：
...
> **使用建议**：
>
> - **适合场景**：简单的双分支赋值，条件判断和返回值都很简单时使用
> - **不适合场景**：复杂的逻辑判断、需要多行代码的情况
> - **可读性**：三元运算符虽然简洁，但可读性不如 if-else，建议初学者优先使用 if-else，熟练后再使用三元运算符
...
- **三元运算符（条件运算符）**：简化的双分支结构
  - **语法**：`条件 ? 值1 : 值2`，如果条件为真返回 `值1`，否则返回 `值2`
  - **示例**：`int max = (a > b) ? a : b;` 等价于 `if (a > b) max = a; else max = b;`
  - **类比**：就像"如果下雨就带伞，否则带雨衣"，用三元运算符就是 `下雨 ? 伞 : 雨衣`
  - **使用场景**：简单的双分支赋值，不适合复杂的逻辑判断
  - **可读性**：虽然简洁，但可读性不如 if-else，建议初学者优先使用 if-else
