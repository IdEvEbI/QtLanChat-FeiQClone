...
**while 循环**：根据条件重复执行代码块的结构。

**工作原理**：只要条件为真，就会一直执行循环体中的代码。条件为假时，循环停止。
...
1. **避免无限循环**：必须在循环体内改变条件，否则会永远执行
   - 正确：`count++`、`number++`、`value--` - 类比：就像忘记关水龙头，水会一直流
   - 错误：忘记更新变量，导致条件永远为真

2. **循环变量初始化**：要在循环外初始化循环变量
   - 正确：`int count = 1;` 然后 `while (count <= 5)` - 类比：就像开车前要检查油箱，从起点（1）开始
...
1. **关于 while 循环，下列说法正确的是：**
   ...
   B. while 循环条件为假时，会继续执行循环体

   C. while 循环条件为假时，退出循环

   D. while 循环不能嵌套使用
   **答案**：C

   **解析**：
   - **正确答案 C**：while 循环条件为假时，会退出循环，不再执行循环体。就像"条件不满足，停止重复"
   - **错误答案 A**：while 循环是"先判断后执行"，如果条件一开始就为假，循环体一次都不会执行
   - **错误答案 B**：while 循环条件为假时会退出循环，不会继续执行
   - **错误答案 D**：while 循环可以嵌套使用，就像套娃，一个循环里可以有另一个循环
...
2. **以下代码会输出多少次 "Hello"？**
   ...
   **解析**：
   - **正确答案 C**：循环从 `i = 1` 开始，每次执行后 `i++`（i 加 1），所以会执行 5 次（1, 2, 3, 4, 5），输出 5 次 "Hello"
   - **错误答案 A**：循环条件是 `i <= 5`，i=1 满足条件，所以至少会执行一次
   - **错误答案 B**：i 从 1 到 5，总共 5 次，不是 4 次
   - **错误答案 D**：因为有 `i++` 改变条件，所以不会无限循环
...
- Q3：如何避免无限循环？
  - **A：**大多数情况下，确保循环体内有改变条件的语句；特殊情况下（如服务器程序、事件循环）使用 `while(true)` 有意创建无限循环，并用 `break` 或 `return` 退出。就像"走几步就停下来"（改变变量），或者"遇到障碍就停止"（break 语句）
- Q4：什么时候可以使用 `while(true)` 无限循环？
  - **A：**`while(true)` 无限循环在以下场景中有意使用：
    - **事件驱动程序**：如 Windows 窗口程序的事件循环，GUI 应用的消息处理循环
    - **服务器程序**：监听网络请求，持续提供服务
    - **游戏主循环**：处理游戏逻辑，持续渲染画面
    - **交互式程序**：菜单系统、命令行界面
  - **关键点**：使用 `while(true)` 时必须有明确的退出机制（`break`、`return`、程序退出条件），否则程序会卡死
