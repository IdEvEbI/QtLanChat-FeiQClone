...
- **为什么这样设计**：C++ 继承了 C 语言的高性能特性，同时添加了面向对象编程能力
- **解决了什么问题**：解决了 C 语言缺乏面向对象特性的问题，同时保持了系统级编程能力
- **有什么优势**：高性能、跨平台、面向对象、系统编程能力强

#### 2.1.2 关键特性

1. **高性能**：编译型语言，执行速度快，适合系统编程
2. **跨平台**：可以在 Windows、macOS、Linux 上运行
3. **面向对象**：支持类、继承、多态等特性
4. **广泛应用**：游戏开发、系统软件、嵌入式系统、Qt 开发
...
- `#include <iostream>`：包含标准输入输出库，提供 `std::cout` 功能
- `int main()`：程序的主函数，程序从这里开始执行
- `std::cout`：标准输出流，用于在控制台显示文本
- `<<`：输出操作符，将内容发送到输出流
- `std::endl`：换行符，相当于 `\n`，同时刷新输出缓冲区
- `return 0`：程序正常结束的标志
...
> **性能提示**：`\n` 比 `std::endl` 性能更好，因为 `std::endl` 会强制刷新缓冲区

### 2.4 原理解析（可选）

#### 2.3.1 程序执行流程

1. **预处理**：处理 `#include` 指令，将头文件内容插入到源文件中
2. **编译**：将 C++ 源代码转换为机器码
3. **链接**：将编译后的目标文件与库文件链接生成可执行文件
4. **运行**：操作系统加载并执行可执行文件
...
> **图表说明**：C++ 程序编译流程图，展示从源代码到可执行文件的完整过程，区分文件类型和处理过程
...
- **为什么选择这种设计**：使用简单的输出语句展示 C++ 程序的基本结构
- **解决了什么问题**：让用户理解 C++ 程序的基本骨架
- **有什么优势**：代码简洁，易于理解，为后续学习打下基础
...
- Q1：C++ 和 C 语言有什么区别？
  - **A：**C++ 是 C 语言的扩展，添加了面向对象编程特性，如类、继承、多态等

- Q2：为什么 C++ 程序需要编译？
  - **A：**C++ 是编译型语言，需要将源代码转换为机器码才能运行

- Q3：`std::cout` 中的 `std::` 是什么意思？
  - **A：**`std` 是标准命名空间，`std::cout` 表示使用标准库中的输出流
