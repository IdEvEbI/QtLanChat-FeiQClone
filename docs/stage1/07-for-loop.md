...
> **预计时间**：40 分钟  
> **难度等级**：⭐⭐  
> **技能收获**：for 循环、嵌套循环、break 和 continue、九九乘法表、循环优化  
> **文档版本**：v1.0  
> **最后更新**：2025-10-26
...
> **类比教学**：for 循环就像"从 1 数到 10"，你事先知道要数多少次（确定的次数）。相比 while 循环的"不知道什么时候停止"（不确定次数），for 循环更适合"要走 10 步"（确定次数）这样的任务。比如做俯卧撑，如果计划做 20 个（确定次数），就用 for 循环；如果是"做到累为止"（不确定次数），就用 while 循环。
...
#### 2.3.5 循环控制语句（break 和 continue）

**break**：立即跳出当前循环，执行循环后的代码。

**continue**：跳过本次循环的剩余部分，继续下一次循环。

> **类比教学**：`break` 就像"遇到紧急情况，立即停止工作"；`continue` 就像"跳过这一项任务，继续处理下一项"。
...
1. **`break` 和 `continue` 的使用场景**：
   - `break` 适用于：找到了结果（如找到了目标值）、错误发生需要停止、满足退出条件
   - `continue` 适用于：跳过某些值（如跳过奇数）、过滤数据、部分条件不满足但继续处理其他数据
2. **`break` 和 `continue` 的区别**：
   - `break`：完全退出循环，不再继续
   - `continue`：跳过本次循环，继续下一次循环检查
...
5. **循环控制**：支持 `break` 和 `continue` 语句，可以灵活控制循环执行流程
...
- **循环控制的作用**：`break` 和 `continue` 提供了灵活的循环控制能力，可以在循环中根据条件立即退出或跳过某些迭代，提高了编程的灵活性和效率
...
> - **数组访问**：`users[0]` 访问第一个元素（小丽）、`users[1]` 访问第二个元素（小美）
...
> **💡 数据溢出提醒**：
>
> - **`long long` 类型限制**：虽然使用了 `long long` 类型存储结果，但当输入大于 25 时（如 26、27），结果会超出 `long long` 的最大值，导致**数据溢出**，输出负数或错误结果
> - **溢出原因**：`long long` 能表示的最大正整数约为 9.2×10¹⁸，而 26! 约等于 4.0×10²⁶，远超此范围
> - **类比**：就像一个只有 9 位数的计算器，计算超过 9 位数的乘法时就会"装不下"（溢出）
> - **建议输入范围**：建议输入 1-20，确保结果正确
>
...
#### 练习 4：使用 break 找到目标数字

**题目**：使用 for 循环和 break 查找目标数字

**要求**：

- 提示用户输入一个目标数字
- 使用 `std::cin` 读取用户输入
- 使用 for 循环从 1 到 20 遍历
- 找到目标数字时使用 `break` 跳出循环
- 输出找到的消息和查找次数
...
> **配套代码**：练习 4 的完整代码位于 `src/stage1/07-for-loop/06-exercise-break.cpp`

#### 练习 5：使用 continue 过滤数字

**题目**：使用 for 循环和 continue 过滤数字

**要求**：

- 使用 for 循环从 1 遍历到 20
- 使用 `continue` 跳过所有 3 的倍数
- 只输出不是 3 的倍数的数字
- 统计并输出跳过的数字个数
...
> **配套代码**：练习 5 的完整代码位于 `src/stage1/07-for-loop/07-exercise-continue.cpp`
...
2. **关于嵌套 for 循环，下列说法正确的是：**
   ...
   **解析**：
   ...
   - **错误答案 C**：内层循环可以使用外层循环的变量（这在 C++ 中是允许的）。比如在九九乘法表中，内层的 j 可以使用外层的 i
...
3. **以下代码会输出什么？**
   ...
   **解析**：
   - **正确答案 C**：循环执行 5 次（i = 1, 2, 3, 4, 5），每次 sum += i \* 2，所以：
     - i=1: sum = 0 + 1\*2 = 2
     - i=2: sum = 2 + 2\*2 = 6
     - i=3: sum = 6 + 3\*2 = 12
     - i=4: sum = 12 + 4\*2 = 20
     - i=5: sum = 20 + 5\*2 = 30
   - **错误答案 A**：如果没有循环，只是 sum = 5 \* 2 = 10，但这是循环累加的结果
   - **错误答案 B**：如果只是 1+2+3+4+5 = 15，但代码中是 i\*2 不是 i
   - **错误答案 D**：代码语法完全正确
...
4. **以下代码会输出什么？**
   ...
   **解析**：
   - **正确答案 B**：当 i=5 时，遇到 `break` 语句，立即跳出循环，所以只输出 1 2 3 4
   - **错误答案 A**：如果有 5，说明没有执行 break，但代码中 i=5 时会执行 break
   - **错误答案 C**：如果输出全部，说明没有执行 break，但代码中 i=5 时会执行 break
   - **错误答案 D**：代码语法完全正确
...
5. **以下代码会输出什么？**
   ...
   **解析**：
   - **正确答案 B**：当 i 是 3 的倍数（3, 6, 9）时，执行 `continue` 跳过本次循环，不输出这些数字，所以输出 1 2 4 5 7 8 10
   - **错误答案 A**：如果输出全部，说明没有执行 continue
   - **错误答案 C**：这是被跳过的数字，不是输出的数字
   - **错误答案 D**：漏掉了 10

### 4.3 常见问题 FAQ

- Q1：for 循环和 while 循环有什么区别？应该用哪个？
  - **A：**for 循环适合**确定次数**的情况（如遍历数组、生成序列），while 循环适合**不确定次数**的情况（如用户输入、条件等待）。就像"走 10 步"用 for，"走到家"用 while。选择原则：知道多少次用 for，不知道用 while
- Q2：嵌套循环有什么用？
...
- Q4：break 和 continue 有什么区别？
  - **A：**`break` 用于立即退出循环（遇到紧急情况，停止工作），`continue` 用于跳过本次循环剩余部分，继续下一次循环（跳过这一项任务，继续处理下一项）
- Q5：什么时候使用 break，什么时候使用 continue？
  - **A：**使用 `break` 的场景：找到了目标值（如找到了目标数字）、错误发生需要停止、满足退出条件。使用 `continue` 的场景：跳过某些值（如跳过奇数）、过滤数据、部分条件不满足但继续处理其他数据
...
> **配套代码**：综合练习的完整代码位于 `src/stage1/07-for-loop/08-homework-multiplication.cpp`
